...
def _check_agent_health(lines: list[str], project_dir: str) -> None:
    ...
    # Try tmux
    try:
        from ai_handoff.session import session_exists, SESSION_NAME
        from ai_handoff.watcher import pane_exists, is_agent_idle
...
def diagnose_state(project_dir: str | None = None,
    ...
    if status != "done" and result:
        ...
        lines.append("       Fix: python -m ai_handoff state set"
    ...
    try:
        from ai_handoff.cycle import read_status as cycle_read_status
...
def state_command(args: list[str]) -> int:
    ...
    print("Usage: python -m ai_handoff state [set|reset|diagnose]")
    return 1
