...
# Import textual at package level so that `from tagteam.tui import tui_command`
...
def tui_command(args: list[str]) -> int:
    """Handle `python -m tagteam tui [--dir PATH] [--sound]`."""
    project_dir = None
    sound = False
    ...
    while i < len(args):
        if args[i] == "--dir" and i + 1 < len(args):
            ...
            print("Usage: python -m tagteam tui [--dir DIR] [--sound]")
            print()
    ...
    if project_dir is None:
        # Check if cwd has tagteam.yaml
        if os.path.exists("tagteam.yaml"):
    ...
    from tagteam.tui.app import SaloonApp
