...
- [x] **Criterion 10 on this repo — DONE 2026-09-20 23:42 PDT.** The restarted iTerm2 watcher's first real dispatch (the Phase 68a plan submission) is in `tagteam watch log`: five `info` idle-probe lines, `Idle detection inconclusive, proceeding after 10s`, then `sent  Sent to codex: Read the handoff contract…`; `watch status` showed that as `last dispatch`. Compared by the lead against the log only — the arbiter's tab prints the same lines by construction (`_log` prints, then records) but nobody read them side by side. Side finding: codex's idle prompt (`gpt-6-astra medium · …`) is not recognised by the idle patterns, so every dispatch to it waits the full 10 s. Earlier note: 2026-09-20 23:15 PDT, after the merge and a watcher restart: `watch status` → `running (pid 35417, mode iterm2)`, `last look: 9s ago`; `watch log` holds the nine start-up lines (`start`, sessions OK, poll trigger, `Current state: done`). No turn has been dispatched since, so the dispatch kinds are still unobserved here. Original note: The iTerm2 watcher running here predates the change. After the merge: restart the watcher tab, run one real handoff round, compare `tagteam watch log` with the tab. The reviewer's approval explicitly does not cover this.
