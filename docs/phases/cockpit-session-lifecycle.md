...
3. **`tagteam serve` stops the watchers it started** (`server.py`,
   `launch.py`): a per-server registry of `(pid, ident)` fed by
   `/api/watch/start` and by Start launches; on shutdown each still-matching
   watcher gets SIGTERM, a bounded wait, and a printed line.
...
`server.py` gets a small registry on the server object:
`started_watchers: list[{pid, ident, source}]`.

- `/api/watch/start`: on `ok` with a `pid`, register it.
- Start launch (`launch.launch` → `_attempt`): when `_attempt` starts a new
  watcher (not `reused`, not `already`), register it. Implemented by passing
  an `on_watcher_started(pid, ident)` callback down from the server; `launch`
  stays usable without it.
- Shutdown (`serve_command`'s `finally`, before `lease.release()`):
  `stop_started_watchers(root, registry)`, for each entry:
  - skip unless the pid is alive **and** `procs.identity(pid)` equals the
    recorded ident (a reused pid is never signalled);
  - SIGTERM, wait up to 10 s for exit (poll `pid_alive`), print
    `Stopped watcher pid N (started by this cockpit).`, or
    `Watcher pid N did not exit within 10 s — stop it with: kill N`;
  - a watcher that is running but not in the registry is not touched; when
    `watcher_status` reports one, print
    `Watcher pid N was not started by this cockpit — left running.`
- The pidfile is removed by the watcher's own `finally` (§2); if the pid is
  gone and the pidfile still names it, `remove_pidfile(root, pid)`.
...
**UI** (`cockpit.js`, the Phase 43 Cycle region + Activity log section, so the
existing node harness can test it):
- `cycleKey(it)` = `phase + '/' + type`; `CURRENT_CYCLE` = the `CYCLE_ID` the
  page already derives from state.
- `RLANE` rows whose `cycleKey` differs from `CURRENT_CYCLE` get a
  `.other-cycle` class and are hidden unless `SHOW_LAST_SESSION` is true.
  Rows are still kept in the store (no refetch needed to reveal them).
...
- Button `#btn-reviewer-last` in the reviewer lane header: label
  **Show last session** / **Hide last session**, `hidden` when there are no
  ...
  (`<phase> · <type>`). The toggle resets to hidden when `CURRENT_CYCLE`
  changes. Not persisted.
- Verdict lookup: `verdictFor(it)` returns a chip only when
  `cycleKey(it) === CURRENT_CYCLE`, keyed by round within that cycle; other
  cycles' rows show no chip (their verdicts are in that cycle's history).
- Lane status text (`renderLanes`) is unchanged: it already describes the
  current state.

## Files
- `tagteam/watcher.py`: lock, refusal, SIGTERM handler
- `tagteam/launch.py`: watcher-started callback in `_attempt`; refusal wording in `start_watcher`
- `tagteam/server.py`: registry, registration in `/api/watch/start` and the launch path, `stop_started_watchers` on shutdown
...
4. SIGTERM to a running `tagteam watch --pidfile` child: it exits, the
   ...
   records it.
5. `serve` shutdown: a watcher registered by `/api/watch/start` (and one by a
   Start launch) is sent SIGTERM and reported stopped; an unregistered live
   watcher is left running and reported; a registered pid whose identity no
   longer matches is never signalled.
...
7. Reviewer lane (node harness): with rows from cycles A and B and
   `CURRENT_CYCLE = B`, only B's rows are visible; the empty state names B
   when B has none; "Show last session" reveals A's rows with their cycle label
   and flips to "Hide last session"; changing `CURRENT_CYCLE` hides them again;
