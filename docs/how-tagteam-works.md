...
tagteam job start ci-watch --pr 59 [--to-lead]  # a background job: wait for CI without a model (list|status|log|cancel)
...
- **config** edits a closed, safe set of `tagteam.yaml` keys: `gatekeeper.enabled`, `gatekeeper.on_submit`, `panel.enabled`, `briefer.enabled` and `watcher.resend_minutes`. Each edit is a targeted line edit, so comments, blank lines and order are kept, and anything unusual is refused rather than guessed at. Before anything is written, the edited text must parse to the old config with only the target changed, and for an enable (or a re-send interval) the engine's own resolver must yield the requested value. Otherwise the edit is refused with the resolver's reason. Turning something off is never refused by that check. A no-op is decided on the *saved* value, so a disable is still written when an invalid block already keeps the feature OFF. Writes take the project writer lock. `--preview` prints the diff and a `base` digest, and `--expect <base>` refuses if the file changed since. An external editor that saves during a write can still lose its change. The Rules tab offers the same edits, showing the diff before it writes.
- **The helper kit** (Phase 73) is three subagents shipped in the Claude Code plugin, for the lead to hand bulky steps to:
  - `tagteam:test-runner` (haiku): one focused test run → a digest; never the full suite, never a fix;
  ...
  None has write tools. The contract requires a delegated helper to run with `TAGTEAM_READ_ONLY=1`, and plugin agents can't set environment variables, so the plugin ships a **PreToolUse hook on Bash**:
  - **Prefilter:** a shell wrapper lets every call that doesn't mention a `tagteam:` agent through at once, with no Python.
  - **Guard:** for a kit call it runs `tagteam hook pre-tool-use`, which decides the identity from the top-level `agent_type`. It exits 0 with the command rewritten to `export TAGTEAM_READ_ONLY=1; …` (every other input field kept, no auto-allow), 3 for "not a kit agent" (the command passes untouched), or 2 to deny.
  - **Validation:** a rewrite is forwarded only after a plugin-side validator checks its structure against the original input.
  - **Everything else is blocked:** no CLI, an older CLI without the guard, empty, truncated or misplaced output, and no `python3`.

  **Verified live** on Claude Code 2.1.281: a helper's unprefixed `echo $TAGTEAM_READ_ONLY` printed `1`, its unprefixed `tagteam cycle add` was refused by the CLI, and the main session's Bash was untouched. An older Claude Code that doesn't report the calling agent can't be guarded at all; `tagteam doctor` warns about that, and about a CLI older than the plugin's `minVersion`.
- **jobs** (Phase 72) are recorded background tasks that involve no model and never take the turn slot. The first kind is `ci-watch`, and each target asks one question. `--pr N` reads `gh pr view --json statusCheckRollup`: an empty rollup means "no checks registered yet" and is never green, and `--expect-checks N` waits until N checks exist. `--run ID` reads `gh run view`. `--workflow NAME --ref REF|--sha SHA` is bound to a **commit**, never a time window: `--ref` is resolved locally once at `start`, the runs of that commit are listed, and the newest is pinned for good, so the run of the previous push can never answer. `--pypi PKG==VER` reads PyPI's `/simple/` index (PEP 691 JSON), because the JSON API lags. `gh` missing, unauthenticated or answering "not found" ends the job as `error` at once, which is different from CI saying `failed`. Records live in files (`.tagteam/jobs/<id>/`), so `job list|status|log` and `GET /api/jobs` never open the database and create nothing. Two OS-released locks decide who may write. The runner holds `runner.lock` for its whole life: a reader that can take the lock proves the runner is gone (`lost`), and one that cannot open it reports `unknown`, which is never treated as dead. Every write of `job.json` re-reads under `job.lock`, and a finished record is never replaced. `cancel` never kills a runner: a live runner stops at its next check, and a lost job is recorded as `cancelled (runner lost)`. Cancel and the timeout are honoured *during* external work, too. A slow `gh` call is killed and a slow PyPI fetch abandoned within 0.2 s, a wait never runs past the deadline, and both are checked again before an answer is committed, so an answer that arrives after a cancel or after the deadline is not accepted. The final poll's attempt count and pinned run are saved with the result. Under `TAGTEAM_READ_ONLY` every job mutator refuses, whether it is reached through the CLI or the cockpit. The result is delivered **at most once, best-effort**, by the process that committed it: a desktop notification (`--quiet` for none; there is no phone transport) and, with `--to-lead`, an interjection by `job:<id>` for the lead's next turn. A process that dies between the commit and the delivery misses the delivery, and nothing replays it. Finished jobs are pruned 7 days after they were created.
...
- **Jobs strip** (Phase 72) — a slim row under the turn bar, present only while a background job is running or finished in the last 24 h (`GET /api/jobs`, file-only). Each chip reads like `ci-watch · PR #59 · running 3m` / `✓ green` / `✗ failed` / `runner lost`. Selecting a chip opens the job's result below the chip: the failing checks with links, the log tail, and what was delivered. A running or lost job has **Cancel** (or **Clear**, for a lost one), confirmed with `tagteam job cancel <id>` through `POST /api/jobs/cancel`. The strip never says anything about whose turn it is. The SSE signature moves with every job write. A runner killed outright leaves no file trace, so its `lost` state appears on the page's next refresh (the 30 s safety net at the latest).
...
| `.tagteam/jobs/<id>/job.json`, `log.txt`, `runner.lock`, `job.lock`, `cancel`, `runner.out` | `tagteam job start` / the detached runner / `job cancel` | one background job: its record, one log line per poll, the runner's lifetime lock, the write lock, the cancel marker and the runner's own output |
